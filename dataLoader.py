...
class SimplePlayByPlayDataset(Dataset):
    def __init__(self, data, sequence_len = 16, shuff=True):
        self.data = data
        ...
        self.len_list = [*range(sequence_len, 90)]
    ...
    def __getitem__(self, idx):
      action_num_data = torch.tensor(np.array(self.data[idx][self.idx_counter - self.sequence_len:self.idx_counter,1:4], dtype=np.float32))
      action_word_data = torch.tensor(np.array([self.action_dict[k] for k in self.data[idx][self.idx_counter - self.sequence_len:self.idx_counter, 4:].flatten().tolist()]))
      ...
      prev_data, next_data = label_data[:-1], label_data[-1]

      if idx == len(self.data) - 1:
        self.idx_counter = self.len_list.pop()
        #print(f'END OF BATCH, idx Counter = {self.idx_counter}, len data = {len(self.data[idx])}')
      if self.len_list == []:
        self.len_list = [*range(self.sequence_len, 90)]
        self.len_list.reverse()
      ...
      return action_data, prev_data, next_data


def get_data_loader_embed(csv_name, batch_size=64, seq_len=10, min_sample=100, split=True, split_ratio=0.7, train_shuffle=True, valid_shuffle=False):
    ...
    train_dataset = SimplePlayByPlayDataset(train_samples, sequence_len=seq_len, shuff=train_shuffle)
    train_dataloader = DataLoader(train_dataset, batch_size=batch_size, shuffle=train_shuffle)

    valid_dataset = SimplePlayByPlayDataset(valid_samples, sequence_len=seq_len, shuff=valid_shuffle)
